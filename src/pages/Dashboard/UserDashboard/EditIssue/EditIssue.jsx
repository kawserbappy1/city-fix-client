...
const EditIssue = () => {
  ...
  const { data: issue, isLoading } = useQuery({
    ...
    onSuccess: (data) => {
      reset(data);
      setImagePreview(data.issueImageURL);
    },
  });
  ...
  const onSubmit = async (data) => {
    try {
      let issueImageURL = issue.issueImageURL;

      // upload image only if changed
      if (data.issueImage && data.issueImage instanceof File) {
        const formData = new FormData();
        formData.append("image", data.issueImage);
        ...
        issueImageURL = res.data.data.url;
      }
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          updateIssueMutation.mutate(updatePayload);
        }
      });
    } catch (err) {
            ...
            {imagePreview ? (
              ...
              <div className="relative">
                <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
                  <img
                    src={imagePreview}
                    defaultValue={issue.issueImageURL}
                    alt="Preview"
